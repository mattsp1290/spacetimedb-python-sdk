...
Current pattern: SIMPLE_WEBSOCKET (/websocket)
"""
...
def patch_spacetime_websocket_client():
    ...
    if "/websocket".count("{") == 0:
        ...
        new_pattern = 'url = f"{protocol}://{host}/websocket"'
    else:
        # Pattern with database in URL
        new_pattern = 'url = f"{protocol}://{host}/websocket".format(database=name_or_address)'
...
def patch_modern_websocket_client():
    ...
    if "/websocket".count("{") == 0:
        # Simple pattern without database in URL
        new_pattern = 'url = f"{protocol_scheme}://{self.host}/websocket"'
    else:
        # Pattern with database in URL
        new_pattern = 'url = f"{protocol_scheme}://{self.host}/websocket".format(database=self.database_address)'
    ...
    if "/websocket".count("{") == 0:
        ...
        subscription_msg = {'subscribe': {'database': 'PLACEHOLDER'}}
        if subscription_msg:
...
def main():
    ...
    print("=" * 40)
    print(f"Applying patch for endpoint pattern: SIMPLE_WEBSOCKET")
    print(f"Endpoint: /websocket")
