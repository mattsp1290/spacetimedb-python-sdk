...
Current pattern: VERSIONED_WS (/v1/ws)
"""
...
def patch_spacetime_websocket_client():
    ...
    if "/v1/ws".count("{") == 0:
        ...
        new_pattern = 'url = f"{protocol}://{host}/v1/ws"'
    else:
        # Pattern with database in URL
        new_pattern = 'url = f"{protocol}://{host}/v1/ws".format(database=name_or_address)'
...
def patch_modern_websocket_client():
    ...
    if "/v1/ws".count("{") == 0:
        # Simple pattern without database in URL
        new_pattern = 'url = f"{protocol_scheme}://{self.host}/v1/ws"'
    else:
        # Pattern with database in URL
        new_pattern = 'url = f"{protocol_scheme}://{self.host}/v1/ws".format(database=self.database_address)'
    ...
    if "/v1/ws".count("{") == 0:
        ...
        subscription_msg = {'subscribe': {'query_strings': ['SELECT * FROM PLACEHOLDER']}}
        if subscription_msg:
...
def main():
    ...
    print("=" * 40)
    print(f"Applying patch for endpoint pattern: VERSIONED_WS")
    print(f"Endpoint: /v1/ws")
