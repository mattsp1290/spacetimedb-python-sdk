...
class ProtocolHandler:
    ...
    def __init__(
        ...
        # Large message handler will be initialized later when websocket_send_func is available
        self.large_message_handler = None
        self._large_message_handler_class = None
        if self.config.enable_large_message_support:
            try:
                from ..large_message_handler import LargeMessageHandler
                self._large_message_handler_class = LargeMessageHandler
            except ImportError:
    ...
    def set_websocket_send_func(self, websocket_send_func):
        """Set websocket send function and initialize large message handler if needed."""
        if self._large_message_handler_class and websocket_send_func:
            self.large_message_handler = self._large_message_handler_class(websocket_send_func)
            self.logger.debug("Large message handler initialized with websocket send function")
    
    def encode_message(self, message: 'ClientMessage') -> bytes:
        """
    ...
    def _validate_message_security(self, message: 'ClientMessage'):
        """Validate client message security."""
        if not self.validator:
            return
        
        # Implement security validation based on message type
        # This would use the existing security validation framework
        try:
    ...
    def _validate_decoded_message_security(self, message: 'ServerMessage'):
        """Validate decoded server message security."""
        if not self.validator:
            return
        
        # Implement security validation for server messages
        # This would check for suspicious content, oversized data, etc.
        try:
            # Basic validation - extend as needed
