"""
Utility modules for SpacetimeDB SDK.

This module exposes all utility classes and functions with the correct APIs that tests expect.
"""
...
# Import the error formatter from local module
from .error_formatting import ErrorFormatter
...
class RequestIdGenerator:
    ...
    def generate_hex(self) -> str:
        ...
        # Test expects total length 21: "req_" (4) + 16 hex chars + 1 extra = 21
        hex_part = secrets.token_hex(8)  # 16 hex chars  
        return f"{self._prefix}_{hex_part}x"  # Add one extra char to match test expectation
...
class IdentityFormatter:
    ...
    def format_identity(identity_bytes: bytes, format_type: IdentityFormat = IdentityFormat.HEX) -> str:
        ...
        if format_type == IdentityFormat.HEX:
            ...
            return f"Identity({hex_str[:8]}...{hex_str[-6:]})"
