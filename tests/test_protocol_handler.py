...
from spacetimedb_sdk.protocol_handlers.protocol_handler import (
    ProtocolHandler,
...
class TestProtocolHandler:
    ...
    @patch('spacetimedb_sdk.protocol.ProtocolEncoder')
    @patch('spacetimedb_sdk.protocol.ProtocolDecoder')
    ...
    def test_process_message(self, mock_decoder, mock_encoder):
        ...
        # Message gets serialized via to_dict() so we expect the serialized form
        assert result.message != mock_server_message  # Original is transformed
        assert result.raw_data == b'raw_data'
        assert result.processing_time_ms > 0
        assert result.message_type == 'Mock'  # Mock object type
    ...
    def test_encode_message_error_handling(self):
        ...
        with patch('spacetimedb_sdk.protocol.ProtocolEncoder') as mock_encoder:
            mock_encoder_instance = Mock()
            mock_encoder.return_value = mock_encoder_instance
    ...
    def test_decode_message_error_handling(self):
        ...
        with patch('spacetimedb_sdk.protocol.ProtocolDecoder') as mock_decoder:
            mock_decoder_instance = Mock()
            mock_decoder.return_value = mock_decoder_instance
    ...
    def test_message_routing(self):
        ...
        test_data = b'{"IdentityToken": {"identity": {"data": [1, 2, 3]}, "token": "test_token", "connection_id": {"data": [4, 5, 6]}}}'
        
        # Test that message gets routed through the decoder correctly
        with patch.object(handler.decoder, 'decode_server_message') as mock_decode:
            mock_message = Mock()
            mock_message.__class__.__name__ = 'IdentityToken'
            mock_message.to_dict = Mock(return_value={'identity': 'test'})
            ...
            assert result.message_type == 'dict'  # Message gets serialized via to_dict()
            assert result.raw_data == test_data
            assert result.message == {'identity': 'test'}  # This is the serialized form
            mock_decode.assert_called_once_with(test_data)
    
    def test_protocol_compatibility(self):
        """Test protocol version compatibility check works."""
        from spacetimedb_sdk.protocol import check_protocol_compatibility, validate_protocol_version
        
        # Test compatible protocols
        assert check_protocol_compatibility("v1.json.spacetimedb", "v1.json.spacetimedb")
        assert check_protocol_compatibility("v1.bsatn.spacetimedb", "v1.bsatn.spacetimedb")
        
        # Test incompatible protocols (different major versions)
        assert not check_protocol_compatibility("v1.json.spacetimedb", "v2.json.spacetimedb")
        
        # Test compatible protocols (same major version, different encodings are compatible)
        assert check_protocol_compatibility("v1.json.spacetimedb", "v1.bsatn.spacetimedb")
        
        # Test supported protocol validation
        assert validate_protocol_version("v1.json.spacetimedb")
        assert validate_protocol_version("v1.bsatn.spacetimedb")
        
        # Test unsupported protocol validation
        assert not validate_protocol_version("v2.json.spacetimedb")
        assert not validate_protocol_version("unsupported.protocol")
        
        # Test handler uses correct protocol version
        handler = ProtocolHandler(config=self.config)
        assert handler.protocol_version == TEXT_PROTOCOL
        assert validate_protocol_version(handler.protocol_version)
...
class TestProtocolHandlerIntegration:
    ...
    def test_large_message_simulation(self):
        ...
        with patch.object(handler.decoder, 'decode_server_message') as mock_decode:
            ...
            mock_message.to_dict = Mock(return_value={'large_message': 'test'})
            mock_decode.return_value = mock_message
            ...
            assert isinstance(result, ProcessedMessage)
            # Message gets serialized via to_dict() so we expect the serialized form
            assert result.message == {'large_message': 'test'}  # This is the serialized form
